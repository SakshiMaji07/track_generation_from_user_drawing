...
- **Live telemetry & lap validation** — in `track_generator_app.py`, the FSDS adapter polls the simulator for car position and collision data. The lap validator tracks checkpoint progression, blocks reverse exploits, and detects lap completion.
...
If any rule is violated, a modal popup blocks further interaction until the canvas is cleared.

## Limitations

- FSDS is **Windows-only**. The adapter uses `CREATE_NEW_CONSOLE` to spawn the simulator in its own window; this flag is no-op on Linux/macOS.
